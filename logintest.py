...
# 페이지 설정
st.set_page_config(page_title="로그인 시스템", layout="wide")

# 세션 상태 초기화
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False

# 간단한 사용자 데이터베이스 (실제 앱에서는 보안 데이터베이스 사용 필요)
users_db = {
    "3wh": sha256("rudckfdbdgkqwjdqhghk3wh".encode()).hexdigest(),
    "3조1": sha256("경찰융합정보화3조1".encode()).hexdigest()
}

def login():
    if username in users_db and users_db[username] == sha256(password.encode()).hexdigest():
...
def login_page():
    ...
    with col2:
        if st.button("회원가입"):
            st.info("현재는 회원가입 기능이 비활성화되어 있습니다.")
